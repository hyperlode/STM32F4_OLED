...
/* Write command */
#define SSD1306_WRITECOMMAND(command)      WRITE_COMMAND(command)
/* Write data */
#define SSD1306_WRITEDATA(data)            WRITE_DATA(data)
/* Absolute value */
...
void TM_SSD1306_UpdateScreen(void) {
	...
	for (m = 0; m < 8; m++) {
		...
		uint16_t count = SSD1306_WIDTH;
		uint8_t temp = 0;
		while (count--) {
			//TM_I2C_WriteData(I2Cx, *data++);
			WRITE_DATA( SSD1306_Buffer[SSD1306_WIDTH*m + temp] );
			temp++;
		}
	}
}
